...
set.seed(49)
index <- createDataPartition(dados_treino$Survived, p = 0.8, list = FALSE)

df_treino <- dados_treino[index,]
df_teste <- dados_treino[-index,]

modelo <- naiveBayes(Survived ~ ., df_treino)

pred <- predict(modelo, df_teste)

table(pred, df_teste$Survived)


x <- paste(round((93+37)/(93+37+13+35) * 100, 2),"%", sep = "")
x
